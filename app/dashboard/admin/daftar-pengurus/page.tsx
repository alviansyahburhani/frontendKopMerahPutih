// Lokasi: app/dashboard/admin/daftar-pengurus/page.tsx
"use client";
...
import { Search, Eye, XCircle, Edit, Trash2, UserPlus } from "lucide-react";
...
const TambahPengurusModal = ({
  ...
  const [jabatan, setJabatan] = useState<"Ketua" | "Sekretaris" | "Bendahara">(
    "Ketua"
  );
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    toast.promise(promise, {
      ...
      error: (err) => {
        ...
        return `Gagal menambahkan pengurus: ${
          ...
          <div>
            ...
            <AsyncSelect
              ...
              getOptionLabel={(option) =>
                `${option.fullName} (NIK: ${option.nik})`
              }
              getOptionValue={(option) => option.id}
              onChange={(option) =>
                setSelectedMember(option as MemberSearchResult)
              }
            ...
            {selectedMember && (
              <p className="text-xs text-green-600 mt-1">
                Terpilih: {selectedMember.fullName}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setJabatan(
                    e.target.value as "Ketua" | "Sekretaris" | "Bendahara"
                  )
                }
...
const EditPengurusModal = ({
  ...
  useEffect(() => {
    if (pengurus) {
      setFormData({
        ...pengurus,
        tanggalDiangkat: pengurus.tanggalDiangkat
          ? new Date(pengurus.tanggalDiangkat).toISOString().split("T")[0]
          : "",
        tanggalBerhenti: pengurus.tanggalBerhenti
          ? new Date(pengurus.tanggalBerhenti).toISOString().split("T")[0]
          : null,
      });
    } else {
      setFormData(null);
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const updateDto: UpdateBoardMemberDto = {
      ...
      alasanBerhenti: formData.tanggalBerhenti
        ? formData.alasanBerhenti || ""
        : null,
    };
          ...
          <div className="border-t pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                onChange={handleChange}
                className="mt-1 w-full p-2 border rounded-md"
                disabled={loading || !formData.tanggalBerhenti}
                placeholder={
                  !formData.tanggalBerhenti
                    ? "(Isi tanggal berhenti terlebih dahulu)"
                    : ""
                }
              />
            </div>
...
const DetailPengurusModal = ({
  ...
  const dataJabatan = [
    ...
    { label: "Tanggal Diangkat", value: formatDate(pengurus.tanggalDiangkat) },
    {
      ...
      value: pengurus.tanggalBerhenti
        ? `Berhenti (${formatDate(pengurus.tanggalBerhenti)})`
        : "Aktif",
    },
  ];
  ...
  const dataPribadi = [
    ...
    {
      label: "Jenis Kelamin",
      value: pengurus.member.gender
        ? pengurus.member.gender === Gender.MALE
          ? "Laki-laki"
          : "Perempuan"
        : "-",
    },
    {
      ...
      value:
        pengurus.member.placeOfBirth && pengurus.member.dateOfBirth
          ? `${pengurus.member.placeOfBirth}, ${formatDate(
              pengurus.member.dateOfBirth
            )}`
          : "-",
    },
  ];
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          <div className="border-b pb-4">
            <h3 className="font-semibold text-brand-red-600 mb-2">
              Informasi Jabatan
            </h3>
...
export default function DaftarPengurusPage() {
  ...
  const [pengurusList, setPengurusList] = useState<BoardMember[]>([]);
  // setter tidak dipakai → kita ambil state saja
  const [tenantInfo] = useState<TenantInfo | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [isTambahModalOpen, setTambahModalOpen] = useState(false);
  const [pengurusToEdit, setPengurusToEdit] = useState<BoardMember | null>(
    null
  );
  const [pengurusToView, setPengurusToView] = useState<BoardMember | null>(
    null
  );
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [boardData] = await Promise.all([
        ...
        // kalau nanti mau ambil tenant: adminService.getTenantInfo()
      ]);
  ...
  const handleBerhentikan = (nama: string, id: string) => {
    if (
      window.confirm(
        `Apakah Anda yakin ingin memberhentikan ${nama} dari jabatannya?`
      )
    ) {
              ...
              <tbody>
                ...
                {!loading && filteredPengurus.length === 0 ? (
                  ...
                  filteredPengurus.map((p) => (
                    <tr
                      key={p.id}
                      className="border-b hover:bg-gray-50 text-sm transition-colors duration-150"
                    >
                      <td className="p-4">
                        ...
                        <p className="text-xs text-gray-500">
                          {p.member.memberNumber || p.member.id}
                        </p>
                      </td>
                      <td className="p-4 font-semibold text-brand-red-700">
                        {p.jabatan}
                      </td>
                      <td className="p-4">
                        {new Date(p.tanggalDiangkat).toLocaleDateString(
                          "id-ID",
                          {
                            day: "2-digit",
                            month: "short",
                      ...
                      <td className="p-4">
                        {p.tanggalBerhenti
                          ? new Date(p.tanggalBerhenti).toLocaleDateString(
                              "id-ID",
                              {
                                day: "2-digit",
                                month: "short",
